function addPassangers(input) {

    let arr = Array.from(input); // creating deep copy . 
    ...
    for (let i = 0; i < arr.length; i++) {
        if (arr[i].includes('Add')) {
            ...
            for (let j = 0; j < wagons.length; j++) {
                if (Number(wagons[j]) + num <= maxCapacity) {
                    wagons[j] = Number(wagons[j]) + num;
                    break;
                }
            }
        }
